...
const INBOX_URLS: Record<string, string> = {
  ...
  // Workspaces (Google / Microsoft hosted — common custom domains)
...
// For company/custom domains, try to detect Google Workspace or Microsoft 365
// via MX record heuristic: use well-known webmail paths as fallback.
function getInboxHref(email: string | null): string {
  ...
  // Exact match in our lookup table
  const direct = INBOX_URLS[domain];
  if (direct) return direct;

  // On mobile, use the native mail app intent
  const isMobile = /android|iphone|ipad|ipod/i.test(navigator.userAgent);
  if (isMobile) {
    // Android Gmail intent works for most Android users
    if (/android/i.test(navigator.userAgent)) {
      return "intent:///#Intent;scheme=gmail;package=com.google.android.gm;end";
    }
    // iOS: message:// opens the native Mail app
    return "message://";
  }

  // Desktop fallback for unknown/company domains: mailto opens default client
  return `mailto:${email}`;
}
...
function getInboxLabel(email: string | null): string {
